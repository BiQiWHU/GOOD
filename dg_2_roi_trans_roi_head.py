...
from mmrotate.core import (build_assigner, build_sampler, obb2xyxy,
                           rbbox2result, rbbox2roi, rbbox_overlaps,
                           hbb2obb)
...
class DG2RoITransRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(DG2RoITransRoIHead, self).__init__(init_cfg)
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        cls_score, bbox_pred, contrast_feat = bbox_head(bbox_feats)
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        for res in sampling_results:
            if stage == 0:
                single_pos_proposal_ious = rbbox_overlaps(                  #rbbox overlaps能不能直接用存疑。
                    hbb2obb(res.pos_bboxes, self.version), res.pos_gt_bboxes, is_aligned=True)
            else:
                single_pos_proposal_ious = rbbox_overlaps(
        ...
        return bbox_results, proposal_ious
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            bbox_results, proposal_ious = self._bbox_forward_train(i, x, sampling_results,
                                                    gt_bboxes, gt_labels,
                                                    rcnn_train_cfg)
            
            bbox_results_clip_styled, _ = self._bbox_forward_train(i, x_clip_styled, sampling_results,
                                                    gt_bboxes, gt_labels,
                                                    rcnn_train_cfg)   
                     
            # consistency loss
            # TODO: Modifie the loss function
            # consistency loss ver 1.0: 
            # import ipdb; ipdb.set_trace()
            original_cls_prob = bbox_results['cls_score'].softmax(1)
            styled_cls_prob = bbox_results_clip_styled['cls_score'].softmax(1)
            loss_consistency_ = 30 * self.jsd(original_cls_prob, styled_cls_prob)
            loss_consistency = {'loss_consistency':loss_consistency_}
            
            # TODO: Modifie the loss funcition
            # contrastive loss ver 1.0:
            # import ipdb; ipdb.set_trace()
            contrast_feat = torch.cat((bbox_results['contrast_feat'], bbox_results_clip_styled['contrast_feat']),
                                      dim=0)
            proposal_ious = torch.cat((proposal_ious, proposal_ious))
            contrast_labels = torch.cat((bbox_results['bbox_targets'][0], bbox_results_clip_styled['bbox_targets'][0]))
            
            loss_contrast = self.bbox_head[i].loss_contrast(
                contrast_feat,
                proposal_ious,
                labels=contrast_labels)            

            bbox_results['loss_bbox'].update(loss_contrast)
            bbox_results['loss_bbox'].update(loss_consistency)
            ...
            for name, value in bbox_results_clip_styled['loss_bbox'].items():
                losses[f's{i}.{name}'+'_clip_styled'] = (
                    value * lw if 'loss' in name else value)
